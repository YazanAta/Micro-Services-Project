...
import { sampleWithRequiredData } from '../governorate.test-samples';
...
describe('Governorate Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.governorates?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to governorateService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getGovernorateIdentifier');
      const id = comp.trackId(entity);
      expect(service.getGovernorateIdentifier).toHaveBeenCalledWith(entity);
