...
import { sampleWithRequiredData } from '../municipality.test-samples';
...
describe('Municipality Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.municipalities?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to municipalityService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getMunicipalityIdentifier');
      const id = comp.trackId(entity);
      expect(service.getMunicipalityIdentifier).toHaveBeenCalledWith(entity);
